...
import math
import torch
from bnn.layers.bbbconv2d import BBBConv2d


class BBBMixtureConv2d(BBBConv2d):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, bias=True, priors=None):
        super(BBBMixtureConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, bias, priors)
        if priors is None:
            priors = {
                'prior_mu': (0, 0),
                'prior_sigma': (1, 0.01),
