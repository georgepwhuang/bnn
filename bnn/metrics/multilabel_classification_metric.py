...
class MultilabelClassificationMetrics(ClassificationMetric):
    ...
    def plot_roc(self, current_epoch):
        ...
        for i in range(rows):
            for j in range(2):
                idx = 2 * i + j
                metrics.RocCurveDisplay(fpr=fpr[idx].cpu().numpy(), tpr=tpr[idx].cpu().numpy(),
                                        roc_auc=auroc[idx]).plot(ax=axs[j, i])
                axs[j, i].set_title(f"Label {self.labels[idx]}")
    ...
    def plot_pr_curve(self, current_epoch):
        ...
        for i in range(rows):
            for j in range(2):
                idx = 2 * i + j
                metrics.PrecisionRecallDisplay(precision=precision[idx].cpu().numpy(),
                                               recall=recall[idx].cpu().numpy()).plot(ax=axs[j, i])
                axs[j, i].set_title(f"Label {self.labels[idx]}")
